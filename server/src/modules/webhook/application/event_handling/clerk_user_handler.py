from modules.user.application.dtos import UserDTO
    ...
    async def handle(self, event: WebhookEventDTO):
        if event.type == "user.created":
            ...
            user_dto = UserDTO(
                ...
                name=event.data.get("first_name", "")
                + " "
                + event.data.get("last_name", ""),
                email=(event.data.get("email_addresses") or [{"email_address": ""}])[
                    0
                ].get("email_address", ""),
            )
